...
export const RaffleProvider = ({ children }: RaffleProviderProps) => {
  ...
  const addToCart = () => {
    if (selectedNumbers.length === 0) {
      toast({
        ...
        description: "Selecione pelo menos um número para adicionar ao carrinho.",
        variant: "destructive",
      });
      return;
    }
    ...
    const newItem: CartItem = {
      ...
      price: selectedNumbers.length * 0.02,
      timestamp: new Date(),
    };
    ...
    toast({
      ...
      description: `${newItem.numbers.length} número(s) adicionado(s) por R$ ${newItem.price.toFixed(2)}`,
    });
  };
  ...
  const totalCartItems = cartItems.reduce((sum, item) => sum + item.numbers.length, 0);
